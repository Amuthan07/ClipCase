"""Offline eval wrapper around clipcase's existing GPT-4o calls. Wraps, does not reimplement."""
...
def generate(frames: list[str], principles: str, api_key: str) -> GenerationResult:
    """Run clipcase's GPT-4o flow-analysis + test-case generation, timed for the eval benchmark.
    ...
    flow_analysis = clipcase.analyze_with_openai(frames, principles, api_key, usage_sink=usage)
    test_cases = clipcase.generate_test_cases(
        flow_analysis, principles, "openai", api_key, usage_sink=usage
    )
